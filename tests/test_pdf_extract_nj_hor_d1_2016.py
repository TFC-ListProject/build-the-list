...
def test_candidates():
    result = pe.candidates(pe.config['sample_hor_nj1.pdf'])
    expected = [
        'Donald W. Norcross',
        'Bob Patterson',
        'Michael Berman',
...
def test_parties():
    result = pe.parties(pe.config['sample_hor_nj1.pdf'])
    expected = [
        'Democratic',
        'Republican',
...
def test_votes_by_municipality_first_row():
    result = pe.votes_by_municipality(extracted_pdf, pe.config['sample_hor_nj1.pdf'])['Burlington County']['Maple Shade Twp']
    expected = [4025, 2752, 109, 64, 53]

    assert result == expected

def test_votes_by_municipality_another_row():
    result = pe.votes_by_municipality(extracted_pdf, pe.config['sample_hor_nj1.pdf'])['Camden County']['Woodlynne Boro']
    expected = [665, 118, 3, 11, 23]

    assert result == expected

def test_votes_by_municipality_second_page_first_row():
    result = pe.votes_by_municipality(extracted_pdf, pe.config['sample_hor_nj1.pdf'])['Gloucester County']['Deptford']
    expected = [7378, 4782, 123, 105, 306]

    assert result == expected

def test_votes_by_municipality_second_page_last_row():
    result = pe.votes_by_municipality(extracted_pdf, pe.config['sample_hor_nj1.pdf'])['Gloucester County']['Federal Overseas']
    expected = [31, 1, 1, 0, 1]
